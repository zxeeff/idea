...
class PopulationRuntimeTest(unittest.TestCase):
    ...
    def test_public_recruitment_joins_after_initial_peers_retire_with_private_copies(self):
        ...
        prepared = prepare_run(forum=self.forum, goal="Improve the example independently", workspace=self.workspace,
                               profiles=self.profiles, population_policy=policy, adaptive=True, workspace_mode="isolated")
        ...
        calls, paths, artifacts = [], [], []

        async def runner(**kwargs):
            agent, invocation = kwargs["agent"], kwargs["invocation"]
            calls.append(str(agent["id"]))
            paths.append(invocation.cwd)
            self.assertNotEqual(self.workspace, invocation.cwd)
            self.assertNotEqual(str(self.forum.state_dir), invocation.env["IDEA_STATE_DIR"])
            ...
            (invocation.cwd / "example.txt").write_text(str(agent["name"]) + "\n")
            artifact = await self.command(invocation, "publish", "--thread", str(thread["id"]), "--note", "Independent change", "--validation", "Read the changed file")
            artifacts.append(artifact)
            await self.command(invocation, "retire", "--reason", "Contribution published")
            return 0
    ...
    def test_invocation_budget_pauses_and_resume_preserves_usage_and_working_copy(self):
        ...
        prepared = prepare_run(forum=self.forum, goal="Continue a bounded run", workspace=self.workspace,
                               profiles=self.profiles[:1], population_policy=policy, adaptive=True, workspace_mode="isolated")
        run_id = str(prepared.run["id"])
